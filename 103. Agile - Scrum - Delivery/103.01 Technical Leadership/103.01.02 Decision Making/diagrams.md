# Diagrams: 103.01.02 Decision Making

Category: Agile / Scrum / Delivery

Topic: 103.01 Technical Leadership

Use these diagrams to understand Decision Making visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.
...
flowchart TD
  ...
  C --> D["Apply Decision Making Rules"]
...
sequenceDiagram
  ...
  participant Core as Decision Making
  participant State
...
Do not optimize Decision Making by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Decision Making"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Decision Making"] --> B{"Clear Owner?"}
