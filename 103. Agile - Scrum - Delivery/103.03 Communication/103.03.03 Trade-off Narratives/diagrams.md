# Diagrams: 103.03.03 Trade-off Narratives

Category: Agile / Scrum / Delivery

Topic: 103.03 Communication

Use these diagrams to understand Trade-off Narratives visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.

## 1. Concept Map

```mermaid
flowchart TD
  A["Agile / Scrum / Delivery"] --> B["103.03 Communication"]
  B --> C["Trade-off Narratives"]
  C --> D["Definition"]
...
flowchart TD
  ...
  C --> D["Apply Trade-off Narratives Rules"]
...
```text
Work enters Trade-off Narratives
...
sequenceDiagram
  ...
  participant Core as Trade-off Narratives
  participant State
...
Do not optimize Trade-off Narratives by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Trade-off Narratives"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Trade-off Narratives"] --> B{"Clear Owner?"}
...
```text
[ ] I can define Trade-off Narratives.
