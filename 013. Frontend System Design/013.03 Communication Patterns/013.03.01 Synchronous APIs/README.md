...
Synchronous APIs is a focused engineering concept inside 013.03 Communication Patterns. It describes a behavior, design choice, implementation technique, or operational concern that engineers must understand deeply to build reliable systems in Frontend System Design.
...
In Frontend System Design, Synchronous APIs commonly appears as a component, hook, service, directive, route, store, style rule, or browser API usage.

Typical shape:

```tsx
type SynchronousAPIsProps = {
  id: string;
  enabled: boolean;
};

export function SynchronousAPIsView(props: SynchronousAPIsProps) {
  const statusLabel = props.enabled ? "active" : "inactive";

  return (
    <section data-topic="Synchronous APIs">
      <h2>{statusLabel}</h2>
    </section>
  );
}
...
Synchronous APIs appears in production anywhere Frontend System Design needs predictable behavior across real users, real traffic, real failures, and real team boundaries.
...
1. What is Synchronous APIs, and why does it matter in Frontend System Design?
