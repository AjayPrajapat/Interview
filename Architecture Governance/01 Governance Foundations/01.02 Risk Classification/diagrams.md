# Diagrams: 01.02 Risk Classification

Category: Architecture Governance

Topic: 01 Governance Foundations

Use these diagrams to understand Risk Classification visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.
...
flowchart TD
  ...
  C --> D["Apply Risk Classification Rules"]
...
```text
Work enters Risk Classification
...
sequenceDiagram
  ...
  participant Core as Risk Classification
  participant State
...
Do not optimize Risk Classification by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Risk Classification"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Risk Classification"] --> B{"Clear Owner?"}
...
```text
[ ] I can define Risk Classification.
