# Diagrams: 03.01 Evidence Collection

Category: Architecture Governance

Topic: 03 Auditability

Use these diagrams to understand Evidence Collection visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.

## 1. Concept Map

```mermaid
flowchart TD
  A["Architecture Governance"] --> B["03 Auditability"]
...
flowchart TD
  ...
  C --> D["Apply Evidence Collection Rules"]
...
```text
Work enters Evidence Collection
...
sequenceDiagram
  ...
  participant Core as Evidence Collection
  participant State
...
Do not optimize Evidence Collection by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Evidence Collection"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Evidence Collection"] --> B{"Clear Owner?"}
...
```text
[ ] I can define Evidence Collection.
