# Diagrams: 006.01.01 Event Loop

Category: Node.js

Topic: 006.01 Node Runtime

Use these diagrams to understand Event Loop visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.
...
sequenceDiagram
  ...
  participant Core as Event Loop
  participant State
...
Do not optimize Event Loop by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Event Loop"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Event Loop"] --> B{"Clear Owner?"}
