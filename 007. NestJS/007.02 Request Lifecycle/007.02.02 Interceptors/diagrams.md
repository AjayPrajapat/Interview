# Diagrams: 007.02.02 Interceptors

Category: NestJS

Topic: 007.02 Request Lifecycle

Use these diagrams to understand Interceptors visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.

## 1. Concept Map

```mermaid
flowchart TD
  A["NestJS"] --> B["007.02 Request Lifecycle"]
  B --> C["Interceptors"]
  C --> D["Definition"]
...
sequenceDiagram
  ...
  participant Core as Interceptors
  participant State
...
Do not optimize Interceptors by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Interceptors"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Interceptors"] --> B{"Clear Owner?"}
