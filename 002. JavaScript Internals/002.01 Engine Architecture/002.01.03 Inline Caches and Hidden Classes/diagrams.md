# Diagrams: 002.01.03 Inline Caches and Hidden Classes

Category: JavaScript Internals

Topic: 002.01 Engine Architecture

Use these diagrams to understand Inline Caches and Hidden Classes visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.

## 1. Concept Map

```mermaid
flowchart TD
  A["JavaScript Internals"] --> B["002.01 Engine Architecture"]
  B --> C["Inline Caches and Hidden Classes"]
  C --> D["Definition"]
...
flowchart TD
  ...
  C --> D["Apply Inline Caches and Hidden Classes Rules"]
...
```text
Work enters Inline Caches and Hidden Classes
...
For JavaScript Internals, pay special attention to:
...
sequenceDiagram
  ...
  participant Core as Inline Caches and Hidden Classes
  participant State
...
Do not optimize Inline Caches and Hidden Classes by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Inline Caches and Hidden Classes"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Inline Caches and Hidden Classes"] --> B{"Clear Owner?"}
...
```text
[ ] I can define Inline Caches and Hidden Classes.
