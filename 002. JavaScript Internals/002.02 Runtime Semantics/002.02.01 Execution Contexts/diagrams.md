# Diagrams: 002.02.01 Execution Contexts

Category: JavaScript Internals

Topic: 002.02 Runtime Semantics

Use these diagrams to understand Execution Contexts visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.
...
flowchart TD
  ...
  C --> D["Apply Execution Contexts Rules"]
...
```text
Work enters Execution Contexts
...
sequenceDiagram
  ...
  participant Core as Execution Contexts
  participant State
...
Do not optimize Execution Contexts by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Execution Contexts"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Execution Contexts"] --> B{"Clear Owner?"}
...
```text
[ ] I can define Execution Contexts.
