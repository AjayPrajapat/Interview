# Diagrams: 002.02.03 Microtasks and Macrotasks

Category: JavaScript Internals

Topic: 002.02 Runtime Semantics

Use these diagrams to understand Microtasks and Macrotasks visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.

## 1. Concept Map

```mermaid
flowchart TD
  A["JavaScript Internals"] --> B["002.02 Runtime Semantics"]
  B --> C["Microtasks and Macrotasks"]
  C --> D["Definition"]
...
flowchart TD
  ...
  C --> D["Apply Microtasks and Macrotasks Rules"]
...
```text
Work enters Microtasks and Macrotasks
...
sequenceDiagram
  ...
  participant Core as Microtasks and Macrotasks
  participant State
...
Do not optimize Microtasks and Macrotasks by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Microtasks and Macrotasks"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Microtasks and Macrotasks"] --> B{"Clear Owner?"}
...
```text
[ ] I can define Microtasks and Macrotasks.
