# Diagrams: 035.01.03 Normalization and Denormalization

Category: Indexing & Query Optimization

Topic: 035.01 Data Modeling

Use these diagrams to understand Normalization and Denormalization visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.

## 1. Concept Map

```mermaid
flowchart TD
  A["Indexing & Query Optimization"] --> B["035.01 Data Modeling"]
  B --> C["Normalization and Denormalization"]
  C --> D["Definition"]
...
flowchart TD
  ...
  C --> D["Apply Normalization and Denormalization Rules"]
...
```text
Work enters Normalization and Denormalization
...
For Indexing & Query Optimization, pay special attention to:

- query latency, lock wait, cache hit ratio, replication lag, cardinality, read/write amplification, and storage growth,
...
sequenceDiagram
  ...
  participant Core as Normalization and Denormalization
  participant State
...
Do not optimize Normalization and Denormalization by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Normalization and Denormalization"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Normalization and Denormalization"] --> B{"Clear Owner?"}
...
```text
[ ] I can define Normalization and Denormalization.
