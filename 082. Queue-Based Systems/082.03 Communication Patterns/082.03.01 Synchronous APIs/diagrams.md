# Diagrams: 082.03.01 Synchronous APIs

Category: Queue/Based Systems

Topic: 082.03 Communication Patterns

Use these diagrams to understand Synchronous APIs visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.
...
flowchart TD
  ...
  C --> D["Apply Synchronous APIs Rules"]
...
sequenceDiagram
  ...
  participant Core as Synchronous APIs
  participant State
...
Do not optimize Synchronous APIs by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Synchronous APIs"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Synchronous APIs"] --> B{"Clear Owner?"}
