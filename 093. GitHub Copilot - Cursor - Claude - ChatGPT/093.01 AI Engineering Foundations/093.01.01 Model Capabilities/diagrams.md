# Diagrams: 093.01.01 Model Capabilities

Category: GitHub Copilot / Cursor / Claude / ChatGPT

Topic: 093.01 AI Engineering Foundations

Use these diagrams to understand Model Capabilities visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.

## 1. Concept Map

```mermaid
flowchart TD
  A["GitHub Copilot / Cursor / Claude / ChatGPT"] --> B["093.01 AI Engineering Foundations"]
  B --> C["Model Capabilities"]
  C --> D["Definition"]
...
flowchart TD
  ...
  C --> D["Apply Model Capabilities Rules"]
...
```text
Work enters Model Capabilities
...
sequenceDiagram
  ...
  participant Core as Model Capabilities
  participant State
...
Do not optimize Model Capabilities by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Model Capabilities"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Model Capabilities"] --> B{"Clear Owner?"}
...
```text
[ ] I can define Model Capabilities.
