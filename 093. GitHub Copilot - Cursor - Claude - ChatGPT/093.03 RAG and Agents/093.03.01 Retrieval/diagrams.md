# Diagrams: 093.03.01 Retrieval

Category: GitHub Copilot / Cursor / Claude / ChatGPT

Topic: 093.03 RAG and Agents

Use these diagrams to understand Retrieval visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.
...
Do not optimize Retrieval by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Retrieval"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Retrieval"] --> B{"Clear Owner?"}
