# Diagrams: 093.03.02 Grounding

Category: GitHub Copilot / Cursor / Claude / ChatGPT

Topic: 093.03 RAG and Agents

Use these diagrams to understand Grounding visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.
...
Do not optimize Grounding by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Grounding"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Grounding"] --> B{"Clear Owner?"}
