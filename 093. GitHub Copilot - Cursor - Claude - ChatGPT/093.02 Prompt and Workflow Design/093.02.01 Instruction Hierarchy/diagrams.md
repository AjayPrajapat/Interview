# Diagrams: 093.02.01 Instruction Hierarchy

Category: GitHub Copilot / Cursor / Claude / ChatGPT

Topic: 093.02 Prompt and Workflow Design

Use these diagrams to understand Instruction Hierarchy visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.

## 1. Concept Map

```mermaid
flowchart TD
  A["GitHub Copilot / Cursor / Claude / ChatGPT"] --> B["093.02 Prompt and Workflow Design"]
...
flowchart TD
  ...
  C --> D["Apply Instruction Hierarchy Rules"]
...
```text
Work enters Instruction Hierarchy
...
For GitHub Copilot / Cursor / Claude / ChatGPT, pay special attention to:

- task success rate, hallucination rate, review quality, latency, cost, token usage, and human override rate,
...
sequenceDiagram
  ...
  participant Core as Instruction Hierarchy
  participant State
...
Do not optimize Instruction Hierarchy by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Instruction Hierarchy"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Instruction Hierarchy"] --> B{"Clear Owner?"}
...
```text
[ ] I can define Instruction Hierarchy.
