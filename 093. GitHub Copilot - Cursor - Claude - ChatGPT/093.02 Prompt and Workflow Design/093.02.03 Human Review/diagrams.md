# Diagrams: 093.02.03 Human Review

Category: GitHub Copilot / Cursor / Claude / ChatGPT

Topic: 093.02 Prompt and Workflow Design

Use these diagrams to understand Human Review visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.
...
sequenceDiagram
  ...
  participant Core as Human Review
  participant State
...
Do not optimize Human Review by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Human Review"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Human Review"] --> B{"Clear Owner?"}
