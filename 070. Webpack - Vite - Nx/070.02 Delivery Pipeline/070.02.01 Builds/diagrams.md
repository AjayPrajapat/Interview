# Diagrams: 070.02.01 Builds

Category: Webpack / Vite / Nx

Topic: 070.02 Delivery Pipeline

Use these diagrams to understand Builds visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.
...
Do not optimize Builds by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Builds"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Builds"] --> B{"Clear Owner?"}
