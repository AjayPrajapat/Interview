# Diagrams: 004.02.01 Change Detection

Category: Angular

Topic: 004.02 Rendering and Reactivity

Use these diagrams to understand Change Detection visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.
...
flowchart TD
  ...
  C --> D["Apply Change Detection Rules"]
...
sequenceDiagram
  ...
  participant Core as Change Detection
  participant State
...
Do not optimize Change Detection by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Change Detection"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Change Detection"] --> B{"Clear Owner?"}
