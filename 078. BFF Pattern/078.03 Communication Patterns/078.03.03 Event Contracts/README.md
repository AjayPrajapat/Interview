...
Event Contracts is a focused engineering concept inside 078.03 Communication Patterns. It describes a behavior, design choice, implementation technique, or operational concern that engineers must understand deeply to build reliable systems in BFF Pattern.
...
In BFF Pattern, Event Contracts commonly appears as a handler, controller, middleware, resolver, guard, interceptor, service, or API contract.

Typical shape:

```ts
type EventContractsInput = {
  id: string;
  payload: unknown;
};

type EventContractsResult =
  | { ok: true; value: unknown }
  | { ok: false; error: string; retryable: boolean };

export function handleEventContracts(
  input: EventContractsInput,
): EventContractsResult {
  if (!input.id) {
...
Event Contracts appears in production anywhere BFF Pattern needs predictable behavior across real users, real traffic, real failures, and real team boundaries.
...
1. What is Event Contracts, and why does it matter in BFF Pattern?
