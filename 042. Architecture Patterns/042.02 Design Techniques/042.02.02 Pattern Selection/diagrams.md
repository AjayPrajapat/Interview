# Diagrams: 042.02.02 Pattern Selection

Category: Architecture Patterns

Topic: 042.02 Design Techniques

Use these diagrams to understand Pattern Selection visually: normal flow, internal lifecycle, memory/resource behavior, failure handling, debugging, optimization, and system design connections.
...
flowchart TD
  ...
  C --> D["Apply Pattern Selection Rules"]
...
sequenceDiagram
  ...
  participant Core as Pattern Selection
  participant State
...
Do not optimize Pattern Selection by intuition alone. Optimize only after measurement identifies the constraint.

## 9. Architecture Decision Map

```mermaid
flowchart TD
  A["Need Pattern Selection"] --> B{"Scope Small?"}
...
```mermaid
flowchart TD
  A["Proposal Uses Pattern Selection"] --> B{"Clear Owner?"}
...
```text
[ ] I can define Pattern Selection.
